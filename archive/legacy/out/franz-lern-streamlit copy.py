...
def create_cloze_text(vocab_list, level):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-2024-05-13",
        messages=[
            {"role": "system", "content": f"Erstelle einen französischen Lückentext für das Sprachlevel {level} mit den folgenden Vokabeln: {', '.join(selected_vocab)}. Gib davor zu verwendenden Vokabeln im Anschluss in einer zufälligen Reihenfolge an. {no_answers}"}
        ]
    )
...
def create_translation_task(vocab_list, level):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-2024-05-13",
        messages=[
            {"role": "system", "content": f"Erstelle deutsche Sätze für das Sprachlevel {level}, die die folgenden französischen Vokabeln enthalten und zur Übersetzung ins Französische geeignet sind. Gebe die französischen Sätze nicht an: {', '.join(selected_vocab)}.{no_answers}"}
        ]
    )
    return response.choices[0].message.content.strip()
...
def create_sentence_building_task(vocab_list, level):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-2024-05-13",
        messages=[
            {"role": "system", "content": f"Erstelle französische Sätze mit den folgenden Wörtern: {words}. {no_answers}"}
        ]
    )
...
def create_error_detection_task(vocab_list, level):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-2024-05-13",
        messages=[
            {"role": "system", "content": f"Erstelle fehlerhafte französische Sätze mit den folgenden Vokabeln, die für das Sprachlevel {level} geeignet sind. Gebe die korrekten Sätze nicht an: {', '.join(selected_vocab)}."}
        ]
    )
    return response.choices[0].message.content.strip()
...
def create_conjugation_task(vocab_list, level):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-2024-05-13",
        messages=[
            {"role": "system", "content": f"Konjugiere das Verb '{verb}' in den folgenden Zeiten: Präsens, Präteritum, Futur, Perfekt. {no_answers}"}
        ]
    )
...
# Streamlit App
st.title("Französisch-Lernprogramm")
print("\n\nFranzösisch-Lernprogramm - Start")
...
task_type = st.selectbox("Wählen Sie eine Übung:", 
    ["", "Schreiben eines Textes und danach Korrektur", "Erstellen eines französischen Lückentextes", 
...
if task_type == "Schreiben eines Textes und danach Korrektur":
...
elif task_type == "Erstellen eines französischen Lückentextes":
    ...
    if st.session_state.vocab_list:
        ...
        for word, translation in quiz.items():
            answer = st.text_input(f"Was ist die französische Bedeutung von '{translation}'?", key=word)
